...
class FreeStreamer(object):
    ...
    """
    def __init__(self, initial, grid_max, time, vfs):
        ...
        if time <= 0.0:
            raise ValueError('freestreaming duration must be > 0.')


        if vfs > 1.0 or vfs < 0.0:
        ...
        X = np.subtract.outer(xy, vfs*time*cos_phi)
        Y = np.subtract.outer(xy, vfs*time*sin_phi)
        ...
        u, v, K = zip(*[
            ...
            (0, 1, vfs*cos_phi),
            (0, 2, vfs*sin_phi),
            (1, 1, vfs*vfs*cos_phi*cos_phi),
            (1, 2, vfs*vfs*cos_phi*sin_phi),
            (2, 2, vfs*vfs*sin_phi*sin_phi),
        ])
        ...
        self._trace = None
        self._shear_tensor_eta_eta = None
    ...
    def grid(self):
        """
        Grid of the Energy-momentum tensor T^μν.
    ...
    def _compute_viscous_corrections(self):
        ...
        # Compute and save the trace of T^μν.
        self._trace = np.zeros(self._Tuv.shape[:2])
        self._trace = np.einsum('uv,...uv', g, T)
    ...
    def total_pressure(self):
        ...
        # Compute (P + Π).
        self._total_pressure = self._total_pressure
    ...
    def shear_tensor_eta_eta(self):
        ...
        # Compute π^\eta\eta = - ( P + Π )/tau².
        self._shear_tensor_eta_eta = - self._total_pressure/self._time**2

        return self._shear_tensor_eta_eta   
